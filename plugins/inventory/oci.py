...
DOCUMENTATION = """
    ...
    options:
        ...
        expand_hostvars:
            description: Run extra commands on each host to fill in additional information about the host.
                         May interrogate volume and network services and can be expensive for people with many hosts.
            type: boolean
        fetch_db_hosts:
...
class InventoryModule(BaseInventoryPlugin, Constructable, Cacheable):
    ...
    def setup_clients(self, regions):
        ...
        self._blockstorage_clients = dict(
            (region, self.create_service_client(BlockstorageClient, region=region))
            for region in self.regions
        )
    ...
    def build_inventory_for_instance(self, instance, region):
        ...
        try:
            ...
            blockstorage_client = self.get_blockstorage_client_for_region(region)
            ...
            if self.params["expand_hostvars"]:
                instance_vars["volumes"] = list()
                instance_vars["volume_attachments"] = list()

                volume_attachments = [
                    volume_attachment
                    for volume_attachment in oci_common_utils.list_all_resources(
                        target_fn=compute_client.list_volume_attachments,
                        compartment_id=compartment.id,
                        instance_id=instance.id,
                    )
                    if self.filter_resource(
                        volume_attachment, lifecycle_state=self.LIFECYCLE_ATTACHED_STATE
                    )
                ]

                for volume_attachment in volume_attachments:

                    volume = oci_common_utils.call_with_backoff(
                        blockstorage_client.get_volume, volume_id=volume_attachment.volume_id
                    ).data
                    self.debug(
                        "Volume {0} is attached to instance {1}.".format(
                            volume.id, volume_attachment.instance_id
                        )
                    )

                    if instance_vars.get("id") == volume_attachment.instance_id:
                        instance_vars["volumes"].append(to_dict(volume))
                        instance_vars["volume_attachments"].append(to_dict(volume_attachment))
    ...
    def get_blockstorage_client_for_region(self, region):
        if region not in self._blockstorage_clients:
            raise ValueError(
                "Could not fetch the block storage client for region {0}.".format(region)
            )
    ...
    def parse(self, inventory, loader, path, cache=True):
        ...
        # Expand hostvars
        if self.get_option("expand_hostvars") is not None:
            self.params["expand_hostvars"] = self.get_option("expand_hostvars")
