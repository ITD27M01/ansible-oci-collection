...
def get_secrets_client(oci_config, signer):
    return vault.VaultsClient(config=oci_config, signer=signer)


def get_secret_bundle_client(oci_config, signer):
    return secrets.SecretsClient(config=oci_config, signer=signer)


def get_secret(config, signer, compartment_id, vault_id, secret_name):
    secrets_client = get_secrets_client(config, signer)
    return secrets_client.list_secrets(compartment_id=compartment_id,
                                       vault_id=vault_id,
                                       name=secret_name).data


def get_secret_data(config, signer, secret):
    secret_bundle_client = get_secret_bundle_client(config, signer)
